    ...
    _.merge(
      {
        ...
        left:
          self.options.multiLine && !self.gutter.options.hidden
            ? self.gutter.width +
              (self.options.hasOwnProperty('gutterGap')
                ? self.options.gutterGap
                : 1)
            : 0,
        right: 0,
        bottom: 0
...
Editor.parseCoordinate = function(n) {
...
Editor.prototype.toggleInsertMode = function() {
  ...
  lines.some(function(line, row) {
    ...
    while (true) {
      ...
      if (!!match[1] === start) {
        ...
        var isOppositeBracket = !!match[
          oppositeBracketMatchIndex(matches.pop())
        ];
...
Editor.prototype._initHighlighting = function() {
  ...
  self.data.highlight = {
    ranges: [],
    revision: 0,
    bucket: highlightClient.getBucket()
  };

  if (!Editor.count++)
    Editor.highlightClient = highlightClient().tap(function(client) {
...
Editor.prototype._updateContent = function() {
  ...
  util.text
    ...
    .forEach(function(line, row) {
      ...
      var gutterLine =
        _.padStart(row + 1, lineNumberWidth) + _.repeat(' ', gutterWidth);
